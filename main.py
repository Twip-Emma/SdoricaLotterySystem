...
LastEditTime: 2025-03-08 18:19:43
...
# 用户练度
@app.get("/getUserTrain")
async def get_user_train(userId: str = Query(..., min_length=1, max_length=30, regex="^[0-9]+$", description="用户QQ号")):
    try:
        image_path = await user_ill.get_user_train(userId)
        return {"code": 200, "data": image_path, "message": "获取成功"}
    except Exception as e:
        return {"code": 500, "data": None, "message": str(e)}
...
async def get_all_rank():
    try:
        image_path = await user_ill.get_all_rank()
        return {"code": 200, "data": image_path, "message": "获取成功"}
    except Exception as e:
        return {"code": 500, "data": None, "message": str(e)}
